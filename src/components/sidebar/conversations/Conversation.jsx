...
function Conversation({ convo, socket, online, typing }) {
    ...
    return (
        <li onClick={() => openConversation()} className={`list-none h-[72px] w-full dark:bg-dark_bg_1 ${convo._id === activeConversation._id ? "" : "hover:dark:bg-dark_bg_2"} cursor-pointer dark:text-dark_text_1 px-[10px] ${convo._id === activeConversation._id ? "dark:bg-dark_hover_1" : ""}`}>
            <div className="relative w-full flex items-center justify-between py-[10px]">
                {/*Left Side */}
                <div className="flex items-center gap-x-3">
                    <div className={`relative min-w-[50px] max-w-[50px] h-[50px] rounded-full overflow-hidden ${online ? 'online' : ''}`}>
                        <img src={getConversationPicture(user, convo.users)} alt="profile" className="w-full h-full object-cover" />
                    </div>
