...
export default function Picture({ readablePicture, setReadablePicture, setPicture }) {
    ...
    const handlePicture = (e)=>{
        ...
        if(pic.type!=="image/jpeg" && pic.type!=="image/png" && pic.type!=="image/jpg" && pic.type!=="image/webp"){
            setError(`Please upload a valid image, ${pic.name} format is not supported`)
            return;
        }else if (pic.size>1024*1024*5){
            setError(`${pic.name} is too large, maximum 5mb is allowed`)
            return;
        }else{
            setError("")
            setPicture(pic);
        ...
        <div className='mt-8 content-center dark:text-dark_text_1 space-y-1'>
            ...
            {readablePicture ? (
                <div>
                    ...
                    <div className='mt-2 w-20 py-1 dark:bg-dark_bg_3 rounded-md text-xs font-bold flex items-center justify-center cursor-pointer' onClick={()=>handleChangePic()}>Remove</div>
                </div>
            ) : (
